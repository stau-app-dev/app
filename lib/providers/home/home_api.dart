...
class HomeApi {
  static Future<Either<Failure, int?>> getDayNumber() async {
    try {
      Response res = await get(Uri.parse(getDayNumberEndpoint));
  ...
  static Future<Either<Failure, SpiritMeters>> getSpiritMeters() async {
    try {
      Response res = await get(Uri.parse(getSpiritMetersEndpoint));
      if (res.statusCode == 200) {
  ...
  static Future<Either<Failure, VerseOfDay>> getVerseOfDay() async {
    try {
      Response res = await get(Uri.parse(getVerseOfDayEndpoint));
      if (res.statusCode == 200) {
